...
% Task-2: simualtion for 10 iteration with 400 individuals
...
% program description:>>> 
% Constructing the stacked area plot for 10 iterations. considering the
% number of individuals to be 400.
...
no_of_per= 400;      % total number of individuals taken for simulation
...
% pramaters for obtaining the data required for task-3 histogram plot
iterations=10;
time_stamp_1=0.3;
time_stamp_2=0.8;
...
vert_limit_end=4;       % vertical limit end

%% Creating all the usefull vectors for calculation

% Creating a position vector for 400 individuals:

x_position= 2*rand(no_of_per,1);  % using rand built-in function for generating random x coordinates for individuals
y_position= 4*rand(no_of_per,1);    % using rand built-in function for generating random y coordinates for individuals
position= [x_position y_position];  % combining x and y coordinates and building a position vector for 400 individuals


% Creating velocity vector for 400 individuals:
...
% using for loop for collecting the simulation data for 10 iterations
for h=1:iterations
    ...
    for i=1:length(time)            
        ...
        % Calculating new position after ecah time step for 400 points 
        ...
        for j=1:no_of_per           
            ...
            % then decrementing the recovery time.
        ...
        % obtaining the values of healthy_percantage at time t=0.3 and
        % t=0.8
        if time_stamp_1==time(i)
            health_per_time1= (healthy/no_of_per);
        elseif time_stamp_2==time(i)
            health_per_time2= (healthy/no_of_per);
        end
...
%          set(gcf, 'units','normalized','outerposition',[0 0 1 1]);
   ...
   % saving the data after each iterations into ".mat" files.
    save(['Pandemic_400_' num2str(h) '.mat'],'health_per_time1','health_per_time2')
    
    figure(2)
    subplot(2,5,h)
    % combining all the vectors to form amatrix for stacked area plot
    matrix= [healthy_vect infected_vect recovered_vect];
